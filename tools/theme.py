"""Resolve Alacritty TOML colors into a C header during the build."""
...
def find_config():
    ...
    candidates = (
        ...
        home / ".alacritty.toml",
        Path("/etc/alacritty/alacritty.toml"),
    )
...
def theme_header(config):
    ...
    bold_bright = colors.get("draw_bold_text_with_bright_colors", False)
    if type(bold_bright) is not bool:
        raise ValueError("colors.draw_bold_text_with_bright_colors must be boolean")
    lines = ["/* Generated by manage.py from Alacritty colors. Do not edit. */",
