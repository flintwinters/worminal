...
Current job: turn the proven one-session shared-view prototype into a shared set of tabs by multiplexing PTYs and retaining tabs without visible views. Keep window-manager proofs on private Xvfb, including remote Cinnamon, while desktops are in use; then measure launch and memory before adopting the feature.
